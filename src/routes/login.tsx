...
export const Route = createFileRoute("/login")({
	...
	loader: async ({ deps: { redirect: redirectTo } }) => {
		const session = await getSession();
		if (session?.user) {
			throw redirect({ to: redirectTo ?? DEFAULT_REDIRECT_AFTER_LOGIN });
		}
	},
});
									...
									<FieldGroup>
										<Field>
											<Button
												variant="outline"
												type="button"
												className="h-11 transition-all hover:border-primary/50 hover:bg-primary/10"
												onClick={() =>
													signIn.social({
														provider: "github",
														callbackURL:
